...
class CNRichTextEditor extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      this.focusOnNextUpdate != -1 &&
      this.textInputs.length > this.focusOnNextUpdate
    ) {
  ...
  handleOnBlur = (e, index) => {
    if (this.props.onBlur) this.props.onBlur(e, index);
  };
  ...
  handleOnFocus = (e, index) => {
    ...
    if (this.props.onFocus) this.props.onFocus(e, index);
  };
  ...
  addImageContent = (url, id, height, width) => {
    ...
    if (newConents[index - 1] && newConents[index - 1].component === "text") {
      ...
      if (Array.isArray(before) && before.length > 0) {
        ...
        if (
          before[before.length - 1].text === "\n" &&
          before[before.length - 1].readOnly !== true
        ) {
          beforeContent.content = update(before, {
  ...
  removeImage = index => {
    ...
    if (content.component === "image") {
      ...
      if (
        ...
        if (
          this.textInputs[index + 1].textLength > 0 &&
      ...
      if (this.props.onRemoveImage) {
        this.props.onRemoveImage({ id: removedId, url: removedUrl });
      }
    }
  };
    ...
    return (
      <View
        ...
        style={{
          ...
          backgroundColor:
            this.state.imageHighLightedInex === index
              ? "yellow"
              : "transparent",
          paddingLeft: 2,
          paddingRight: 2,
        ...
        <TouchableWithoutFeedback onPress={() => this.onImageClicked(index)}>
          <ImageComponent
            style={{
              width: myWidth,
              height: myHeight,
  ...
  onMeasureLayout = event => {
    ...
    const measureOffset = Math.ceil(
      Math.max(0, measureRequiredHei - this._rootHei)
    );

    if (
      this._rootHei < measureRequiredHei &&
