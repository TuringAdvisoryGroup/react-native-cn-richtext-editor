...
class CNTextInput extends Component {
  ...
  onSelectionChange = event => {
    ...
    if (
      ...
      if (upComingStype !== null) {
        if (
          upComingStype.sel.start === this.prevSelection.start &&
  ...
  handleChangeText = text => {
    ...
    if (IS_IOS) {
      if (this.prevSelection.end !== this.prevSelection.start) {
        ...
        Math.abs(this.prevSelection.end - selection.end) == Math.abs(textDiff)
      ) {
        cursorPosition = this.prevSelection.end;
      } else if (
        Math.abs(this.prevSelection.end - selection.end) +
    ...
    if (shouldRemText === true) {
      preparedText =
        preparedText.substring(0, cursorPosition - remDiff) +
    ...
    if (shouldAddText === true) {
      ...
      preparedText =
        preparedText.substring(0, cursor) +
        addedText +
        preparedText.substring(cursor);
    }

    if (preparedText === myText) {
      if (shouldRemText === true) {
        const result = this.removeTextFromContent(
          content,
          cursorPosition,
          remDiff
        );
      ...
      if (shouldAddText === true) {
        ...
        const addedText = text.substring(
          cursorPosition,
          cursorPosition + addDiff
        );
      ...
      for (let index = 0; index < mydiff.length; index++) {
        ...
        switch (element[0]) {
          ...
          case -1:
            ...
            result = this.removeTextFromContent(
              content,
              myIndex,
              element[1].length
            );
  ...
  addTextToContent(content, cursorPosition, textToAdd) {
    ...
    if (
      ...
      const { findIndx, itemNo } = this.findContentIndex(
        content,
        cursorPosition
      );
      foundIndex = findIndx;
    ...
    content[foundIndex].text =
      content[foundIndex].text.substring(0, foundItemNo) +
      textToAdd +
      content[foundIndex].text.substring(foundItemNo);
    ...
    if (newLineIndex >= 0) {
      ...
      content[foundIndex].text.substring(0, 1) == "\n" &&
  ...
  removeTextFromContent(content, cursorPosition, removeLength) {
    ...
    if (foundItemNo >= remDiff) {
      ...
      content[foundIndex].text =
        txt.substring(0, foundItemNo - remDiff) +
      ...
      content[foundIndex].text = content[foundIndex].text.substring(
        foundItemNo
      );
      ...
      if (rem > 0) {
        for (var i = foundIndex - 1; i >= 0; i--) {
          ...
          if (content[i].len >= rem) {
            content[i].text = content[i].text.substring(
              0,
              content[i].len - rem
            );
            content[i].len -= rem;
  ...
  updateNewLine(content, index, itemNo) {
    ...
    if (itemNo === 0) {
      ...
      newContent[index].text = foundElement.text.substring(
        0,
        foundElement.text.length - 1
      );

      newContent[index].NewLine =
        newContent[index].text.indexOf("\n") === 0 || index === 0;
      if (
        ...
        beforeContent.styleList = StyleSheet.flatten(
          this.convertStyleList(
            update(beforeContent.stype, { $push: [beforeContent.tag] })
          )
        );
      ...
      beforeContent = {
        ...
        text: foundElement.text.substring(0, itemNo),
        NewLine:
          foundElement.text.substring(0, itemNo).indexOf("\n") === 0 ||
          index === 0
      };
  ...
  createUpComing(start, end, tag, stype) {
    this.upComingStype = {
      ...
      styleList: StyleSheet.flatten(
        this.convertStyleList(update(stype, { $push: [tag] }))
      )
    };
  }
        ...
        if (tag === "ul") {
          ...
          if (content[i].readOnly === true) {
            ...
            if (content[i].len > (i === 0 ? 0 : 1)) {
              ...
              content[i].NewLine = false;
              listContent = {
                // this is where the list content gets built
                id: shortid.generate(),
                len: i === 0 ? 3 : 4,
                stype: [],
                text: i === 0 ? "1- " : "\n1- ",
